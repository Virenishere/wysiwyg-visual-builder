...
import Toolbar from '@/components/CustomRichTextEditor/Toolbar';
...
const EditableTextElement = ({
  ...
  useEffect(() => {
    if (editorRef.current && element.content !== editorRef.current.innerHTML) {
    ...
    setTimeout(() => {
      try {
        ...
        if (success && editorRef.current) {
          updateElementContent(element.id, editorRef.current.innerHTML);
      ...
      case 'undo':
        if (historyIndex > 0) {
          ...
          if (editorRef.current) {
            ...
            updateElementContent(element.id, prevContent);
            editorRef.current.focus();
          }
        }
        break;
      case 'redo':
        if (historyIndex < history.length - 1) {
          ...
          if (editorRef.current) {
            ...
            updateElementContent(element.id, nextContent);
            editorRef.current.focus();
          }
        }
        break;
  ...
  const handleBlur = (e) => {
    // Don't immediately set editing to false if clicking on toolbar
    const relatedTarget = e.relatedTarget;
    const toolbar = document.querySelector('.toolbar-container');

    if (relatedTarget && toolbar?.contains(relatedTarget)) {
      // Keep editing mode and maintain focus
      setTimeout(() => {
        if (editorRef.current) {
          editorRef.current.focus();
        }
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      <div
        ...
        onKeyUp={handleKeyUp}
        onMouseDown={(e) => {
          // Prevent drag when clicking in text editor
          e.stopPropagation();
        }}
        style={{
          width: '100%',
