"use client"
import { FiMaximize2, FiMaximize, FiArrowRight, FiArrowDown } from "react-icons/fi"

export default function PositionSize({ selectedElement, updateElement, parentId, boxId, elementId }) {
  const fields = [
    { label: "Width", key: "width", icon: <FiMaximize className="w-4 h-4" />, max: 1000 },
    { label: "Height", key: "height", icon: <FiMaximize2 className="w-4 h-4" />, max: 1000 },
    { label: "X Position", key: "x", icon: <FiArrowRight className="w-4 h-4" />, max: 500 },
    { label: "Y Position", key: "y", icon: <FiArrowDown className="w-4 h-4" />, max: 500 },
  ]
    ...
    <div className="mb-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        {fields.map(({ label, key, icon, max }) => (
          <div key={key} className="p-4 bg-gradient-to-br from-orange-50 to-red-50 rounded-xl border border-orange-100">
            ...
            <div className="space-y-2">
              <input
                ...
                onChange={(e) =>
                  updateElement(parentId, boxId, elementId, { [key]: Number.parseInt(e.target.value) || 0 })
                }
