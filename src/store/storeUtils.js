...
export const validateTemplate = (template) => {
  ...
  for (let i = 0; i < template.parents.length; i++) {
    ...
    for (let j = 0; j < parent.rnds.length; j++) {
      ...
      if (typeof rnd.width !== 'number' || typeof rnd.height !== 'number' ||
          typeof rnd.x !== 'number' || typeof rnd.y !== 'number') {
        return { valid: false, error: `RND ${j} in parent ${i} has invalid dimensions` };
      }
      ...
      if (rnd.elements && Array.isArray(rnd.elements)) {
        for (let k = 0; k < rnd.elements.length; k++) {
          ...
          if (!element.type || typeof element.type !== 'string') {
            return { valid: false, error: `Element ${k} in RND ${j} of parent ${i} has invalid type` };
          }

          if (typeof element.x !== 'number' || typeof element.y !== 'number' ||
              typeof element.width !== 'number' || typeof element.height !== 'number') {
            return { valid: false, error: `Element ${k} has invalid dimensions` };
          }
        }
      }
    }
